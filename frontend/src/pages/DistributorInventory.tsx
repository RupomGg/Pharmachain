...
export function DistributorInventory() {
  ...
  const fetchInventory = async () => {
    ...
    setLoading(true)
    try {
      // NOTE: Using the same search API but it will filter by 'owner' (which is the distributor's address)
           ...
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-b border-white/10 pb-6">
            <div>
              <h1 className="text-3xl font-bold text-white tracking-tight flex items-center gap-2">
                 Distributor Stock <Badge variant="outline" className="ml-2 border-indigo-500/30 text-indigo-400 bg-indigo-500/10">{batches.length} Items</Badge>
              </h1>
              <p className="text-slate-400 mt-1">Manage your procured inventory.</p>
            </div>
            <div className="flex items-center gap-3">
               <Button 
                  ...
                  disabled={loading}
                  className="bg-slate-900 border-white/10 text-slate-300 hover:text-white hover:bg-white/5 hover:border-indigo-500/50 transition-all font-mono text-xs"
               >
          ...
          <div className="bg-slate-900/80 backdrop-blur-md border border-white/10 rounded-xl p-4 mb-8 sticky top-4 z-20 shadow-2xl flex flex-col lg:flex-row gap-4 justify-between items-center">
             ...
             <div className="relative w-full lg:w-96 group">
                ...
                <Input 
                  ...
                  className="pl-10 bg-slate-950 border-white/10 text-white placeholder:text-slate-600 focus:border-indigo-500/50 focus:ring-indigo-500/20 h-10 transition-all"
                />
             </div>
             ...
             <div className="flex items-center gap-3 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                     <Button variant="outline" className="bg-slate-950 border-white/10 text-slate-300 hover:text-white hover:border-indigo-500/30 w-[140px] justify-between h-10">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                     <Button variant="outline" className="bg-slate-950 border-white/10 text-slate-300 hover:text-white hover:border-indigo-500/30 w-[160px] justify-between h-10">
                        Sort Order
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-in fade-in zoom-in-95 duration-300">
                {batches.map((batch) => (
                    <Card 
                        ...
                        className="bg-slate-900/40 border border-white/10 hover:border-indigo-500/50 transition-all hover:-translate-y-1 hover:shadow-[0_10px_40px_-10px_rgba(99,102,241,0.1)] group overflow-hidden"
                    >
                        {/* Status Badge */}
                        ...
                        <CardContent className="pt-4 space-y-4">
                            <div className="flex justify-between items-end">
                                ...
                                <Activity className="w-5 h-5 text-indigo-500/20" />
                            </div>
                        ...
                        <CardFooter className="pt-0">
                            <Button 
                                className="w-full bg-slate-950 border border-white/10 hover:bg-indigo-500/10 hover:border-indigo-500/50 hover:text-indigo-400 transition-all group/btn" 
                ...
                <Table>
                  <TableHeader className="bg-slate-950 border-b border-white/10">
                    <TableRow className="hover:bg-transparent">
                      <TableHead className="w-[80px] text-indigo-500/70 text-xs uppercase tracking-wider">Asset</TableHead>
                      <TableHead className="text-indigo-500/70 text-xs uppercase tracking-wider">Batch ID</TableHead>
                      <TableHead className="text-indigo-500/70 text-xs uppercase tracking-wider">Product</TableHead>
                      <TableHead className="text-indigo-500/70 text-xs uppercase tracking-wider">Quantity</TableHead>
                      <TableHead className="text-indigo-500/70 text-xs uppercase tracking-wider">Expiry</TableHead>
                      <TableHead className="text-indigo-500/70 text-xs uppercase tracking-wider">Status</TableHead>
                      <TableHead className="text-right text-indigo-500/70 text-xs uppercase tracking-wider">Action</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {loading && batches.length === 0 ? (
                       ...
                       batches.map((batch) => (
                         <TableRow key={batch.manufacturerBatchNo || batch._id} className="group hover:bg-white/5 border-b border-white/5 transition-colors">
                           <TableCell>
                              <div className="w-10 h-10 rounded-md bg-slate-800 overflow-hidden border border-white/10 group-hover:border-indigo-500/30 transition-colors">
                                 {batch.productImage ? (
                           ...
                           <TableCell>
                              <div className="font-mono text-xs text-slate-500 group-hover:text-indigo-400 transition-colors">
